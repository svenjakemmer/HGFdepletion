...
.doLog <- TRUE
.dataFolder       <- "../01-Data"
.resultsFolder     <- "../04-Results/MM-02.0"
...
# .. 1 Observables -----
observables <- eqnvec(
  pMet_au = "scale_pMet * (pMet_HGF) + offset_pMet",
  tMet_au = "scale_tMet * (Met + pMet_HGF) + offset_tMet",
  HGF_au = "HGF"
  )
...
reactions <- NULL %>% 
  ...
  addReaction("Met + HGF", "pMet_HGF", rate = "k_act * Met * HGF", description = "HGF to MET association and activation (no steady state)") %>% 
   addReaction("pMet_HGF", "", rate = "k_degact*pMet_HGF", description = "pMet_HGF degradation") %>% 
